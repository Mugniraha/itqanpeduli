...
@section('konten')
    ...
    <div class="bg-white p-2 mb-2 ">
        <div class="flex justify-end px-4 mb-4">
            <a type="button" href="{{ url('/inputTambahBank') }}"
                class="text-white bg-green-700 p-10 border border-gray-300 focus:outline-none hover:bg-green-600 focus:ring-4 focus:ring-gray-100 font-semibold rounded-lg text-md px-px py-2.5 w-32 h-12 text-center">
                Tambah Bank
            </a>
        </div>
    </div>
    ...
    <div class="grid md:grid-cols-2 sm:grid-cols-1 w-full gap-6">
        {{-- @for ($i = 0; $i == 3; $i++) --}}
        @foreach ($data_banks as $row)
            <div class="w-full bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                <div class="flex flex-col p-5">
                    <div class="flex justify-between">
                        <diV class="md:flex gap-3">
                            <div class="w-32 h-24">
                                <img class="h-24 mb-3 object-contain"
                                src="{{asset('storage/icon_bank/'. $row->icon_bank)}}" alt="{{$row->nama_bank}}" />
                            </div>
                            <div class="ms-3">
                                <h5 class="text-md font-medium text-gray-900 dark:text-white">{{$row->nama_bank}}</h5>
                                <p class="text-sm text-gray-500 dark:text-gray-400">{{$row->nomor_rekening}}</p>
                                <p class="mb-3 text-sm text-gray-500 dark:text-gray-400">A.N. {{$row->nama_pemilik_bank}}</p>
                            </div>
                        </diV>
                        <div class="flex items-start px-4 pt-4">
                            <button id="dropdownButton-{{$row->id}}" data-dropdown-toggle="dropdown-{{$row->id}}"
                            ...
                            <div id="dropdown-{{$row->id}}"
                                ...
                                <ul class="py-2" aria-labelledby="dropdownButton-{{$row->id}}">
                                    <li>
                                        <div class="flex items-center px-5 mb-1">
                                            ...
                                            <a href="{{route('data-bank.edit', $row->id)}}"
                                    ...
                                    <li>
                                        <div class="flex items-center px-5">
                                            ...
                                            <form action="{{route('data-bank.destroy', $row->id)}}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                @method('DELETE')
                                                <button href="" type="submit"
                                                    class="pl-1 block text-sm text-red-600 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Delete</button>
                                            </form>
