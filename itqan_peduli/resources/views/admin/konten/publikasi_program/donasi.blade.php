        ...
        <div class="flex flex-col p-5">
            <div class="flex justify-between">
                ...
                <div class="flex items-start px-4 pt-4">
                    <button id="dropdownButton-{{ $campaign->title }}" data-dropdown-toggle="dropdown-{{ $campaign->title }}" class="inline-block text-gray-500 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg text-sm p-1.5" type="button">
                    ...
                    <!-- Dropdown menu -->
                    <div id="dropdown-{{ $campaign->title }}" class="z-10 hidden text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow w-44">
                        <ul class="py-2" aria-labelledby="dropdownButton-{{ $campaign->title }}">
                        <li>
                            <div class="flex items-center px-5 mb-1">
                                <svg class="w-[15px] h-[15px] text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.779 17.779 4.36 19.918 6.5 13.5m4.279 4.279 8.364-8.643a3.027 3.027 0 0 0-2.14-5.165 3.03 3.03 0 0 0-2.14.886L6.5 13.5m4.279 4.279L6.499 13.5m2.14 2.14 6.213-6.504M12.75 7.04 17 11.28"/>
                                </svg>
                                <a href="{{ route('campaign.edit', $campaign->id) }}" class="pl-1 block text-sm text-gray-700 hover:bg-gray-100">Edit</a>
                            </div>
                        </li>
                        <li>
                            <form method="POST" action="{{ route('campaign.destroy', $campaign->id) }}">
                                ...
                                <div class="flex items-center px-5">
                                    ...
                                    <button type="submit" class="pl-1 block text-sm text-red-600 hover:bg-gray-100">Delete</button>
                                </div>
                            </form>
                        </li>
                        </ul>
            ...
            <h5 class="text-md font-medium text-gray-900">{{ $campaign->title }}</h5>
            <span class="mb-3 text-sm text-gray-500">{{ $campaign->category }}</span>
            ...
            <div class="flex items-center mt-4 md:mt-6">
                <a href="{{ route('campaign.show', $campaign->id) }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300">Preview</a>
            </div>
        </div>
    </div>
    @endforeach
