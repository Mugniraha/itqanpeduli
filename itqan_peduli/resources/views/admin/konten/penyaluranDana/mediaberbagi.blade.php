        ...
        <div class="subheader text-white my-8">
            <div class="welcome flex justify-between bg-green-700 w-full py-8 px-4 rounded-lg">
                <div class="kiri my-auto">
                    <p class="text-2xl font-semibold">Hak Media Berbagi</p>
                    <p class="text-sm text-gray-300 mt-2 font-normal">History penyaluran dana terkumpul</p>
                </div>
                <div class="kanan my-auto">
                    <a href="{{ url('/inputmediaberbagi') }}" class="text-green-700 bg-white p-10 border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-normal rounded-lg text-sm px-5 py-2.5 mb-2">
                        Input Pencatatan
                    </a>
                </div>
            </div>
            ...
            <table class="w-full text-sm text-left text-gray-700 dark:text-gray-400">
                ...
                @foreach ($media as $berbagi)
                    <tr class="odd:bg-white even:bg-gray-50 border-b dark:border-gray-700">
                        ...
                        <td class="px-6 py-4">
                            Rp {{ number_format($berbagi->nominal, 0, ',', '.') }}
                        </td>
                        <td class="px-6 py-4">
                            {{ $berbagi->catatan }}
                        </td>
                        <td class="px-6 py-4">
                            {{ \Carbon\Carbon::parse($berbagi->created_at)->format('d-m-Y') }}
                        </td>
                        <td class="px-6 py-4 flex">
                            <a href="{{ route('edit.media', $berbagi->id) }}" class="text-blue-600 dark:text-blue-500 hover:underline">
                            ...
                            <form action="{{ route('destroy.media', $berbagi->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-600 dark:text-white hover:underline ml-2">
