        ...
        <div class="subheader text-white my-8">
            <div class="welcome flex justify-between bg-green-700 me-30 w-full py-8 px-4 rounded-lg">
                ...
                <div class="kanan my-auto">
                    <a type="button" href="{{ url('/inputberita') }}" class="text-green-700 bg-white p-10 border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-normal rounded-lg text-sm px-5 py-2.5 me-2 mb-2">
                        Buat Update
                    </a>
                </div>
            </div>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg my-6 bg-white p-5">
            <table class="w-full text-sm text-left rtl:text-right text-gray-700">
                ...
                <tbody>
                    @foreach ($berita as $item)
                        <tr class="{{ $loop->odd ? 'odd:bg-white odd:dark:bg-gray-900' : 'even:bg-gray-50 even:dark:bg-gray-800' }} border-b">
                            ...
                            <td class="px-6 py-4">
                                {{ $item->tipe ?? 'update' }} <!-- Default value 'update' -->
                            </td>
                            <td class="px-6 py-4">
                                {{ $item->created_at->format('d-M-Y') }}
                            </td>
                            <td class="px-6 py-4 flex">
                                <a href="{{ route('edit-berita', $item->id) }}" class="font-normal text-blue-600 hover:underline">
                                ...
                                <form action="{{ route('delete-berita', $item->id) }}" method="GET" onsubmit="return confirm('Apakah Anda yakin ingin menghapus berita ini?');" class="inline-block">
            ...
            <nav class="flex items-center flex-column flex-wrap md:flex-row justify-between pt-4" aria-label="Table navigation">
                ...
                <ul class="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
                    ...
                    <li>
                        <a href="#" class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700">Next</a>
                    </li>
                </ul>
            </nav>
        </div>
